...
void attach_pili_to_turbine()
{
    ...
    for (int i=0; i<cell_number*N_pili; i++)
    {
        for (int j=0; j<N_turbine_width*N_turbine_length; j++)
        {
            ...
            {
                std::exponential_distribution<> distribution(rate_turbine_attach);
                if (distribution(generator) < pili_dynamics_time)
                {
                    pili_state[i] = 2;
                    pili_first_attached[i] = 1;
...
int main(int argc, const char * argv[]) {
    /// Get parameters from script - only uncomment on the PKS cluster!
        ...
        time_pili_turbine_de        = atof(argv[6]);
        force_pili_turbine_de       = atof(argv[7]);
        N_turbine_length            = atof(argv[8]);
        N_turbine_width             = atof(argv[9]);
        seed                        = atof(argv[10]);
    /// Steps
        const long int Total_Steps = total_time / time_step;
        const long int Pili_dynamics_Steps  = pili_dynamics_time  / time_step + 0.5;
        const long int Save_Steps = time_save_results / time_step + 0.5 ;
